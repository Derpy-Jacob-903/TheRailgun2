...
public class FerrousPotion : TheRailgun2Potion
{
    ...
    {
        if (Owner.Character.CardPool.GetUnlockedCards(Owner.UnlockState, Owner.RunState.CardMultiplayerConstraint).Count((Func<CardModel, bool>)(c => c.Tags.Contains(EchoOrb.Ferrous))) == 0)
        {
            CardPileAddResult combat2 = await CardPileCmd.AddGeneratedCardToCombat(ModelDb.Card<IronWave>(), PileType.Hand, Owner);
        }
        else
        {
            CardModel card = await CardSelectCmd.FromChooseACardScreen(choiceContext, (IReadOnlyList<CardModel>) CardFactory.GetDistinctForCombat(Owner, Owner.Character.CardPool.GetUnlockedCards(Owner.UnlockState, Owner.RunState.CardMultiplayerConstraint).Where<CardModel>((Func<CardModel, bool>) (c => c.Tags.Contains(EchoOrb.Ferrous))), 
                Math.Min(DynamicVars.Cards.IntValue, Owner.Character.CardPool.GetUnlockedCards(Owner.UnlockState, Owner.RunState.CardMultiplayerConstraint).Count((Func<CardModel, bool>)(c => c.Tags.Contains(EchoOrb.Ferrous)))), Owner.RunState.Rng.CombatCardGeneration).ToList<CardModel>(), Owner, true);
            if (card == null)
                return;
        ...
        foreach (CardModel cardModel in Owner.PlayerCombatState.AllCards
                     .Where<CardModel>((Func<CardModel, bool>)(c => c.Tags.Contains(EchoOrb.Ferrous)))
                     .ToList<CardModel>())
        {
